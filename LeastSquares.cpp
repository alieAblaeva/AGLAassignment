...
public:
    ...
    /*
    ColumnVector operator+(const ColumnVector &another) {
        ColumnVector temp;
        temp = *this;
        for (int i = 0; i < this->elements.size(); i++) {
            temp.elements[i] += another.elements[i];
        }
        return temp;
    }*/
     ...
     * @param another vector to be multiplied by
     * @return result of multiplication of two vectors
     */
     /*
    int operator*(const ColumnVector &another) {
        int result = 0;
        for (int i = 0; i < this->elements.size(); i++) {
            result += this->elements[i] * another.elements[i];
        }
        return result;
    }
*/
    ...
    friend istream &operator>>(istream &input, ColumnVector &vector) {
        ...
        vector.elements.resize(size);
        for (int i = 0; i < size; i++) {
            input >> vector.elements[0][i];
        }
        return input;
    }
      */
    ...
    friend ostream& operator<<(ostream &output, ColumnVector &vector){
        for(int i = 0; i<vector.dimension; i++){
            if(abs(vector[i])<1e-10){
                ...
                output<<fixed<<setprecision(4)<<vector[i]<<endl;
        }
        return output;
    }*/
